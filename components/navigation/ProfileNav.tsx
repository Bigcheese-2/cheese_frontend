...
import { CreditCard as CardIcon, LayoutGrid, User } from "lucide-react";
        ...
        <div className="fixed bottom-6 left-1/2 -translate-x-1/2 w-[calc(100%-16px)] max-w-[448px] bg-[#111111]/80 backdrop-blur-xl border border-white/5 rounded-[32px] px-2 h-20 z-50">
            <nav className="flex items-center justify-between h-full gap-1">
                {NAV_ITEMS.map((item) => {
                    ...
                    const isCard = item.icon === "card";
                    const isActivity = item.icon === "layout-grid";
                    ...
                    const renderIcon = (active: boolean) => {
                        ...
                        if (isCard) return <CardIcon className={iconClasses} strokeWidth={active ? 2.5 : 1.5} />;
                        if (isActivity) return <LayoutGrid className={iconClasses} strokeWidth={active ? 2.5 : 1.5} />;
                        if (isProfile) return <User className={iconClasses} strokeWidth={active ? 2.5 : 1.5} />;
